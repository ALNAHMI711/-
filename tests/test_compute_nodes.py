...
def test_routes_to_least_loaded_capable_online_node() -> None:
    manager = ComputeNodeManager(
        [
            node("busy", capabilities=frozenset({"video"}), current_jobs=1, max_concurrency=2, state=NodeState.ONLINE),
            node("free", capabilities=frozenset({"video"}), max_concurrency=2, state=NodeState.ONLINE),
        ]
    )
...
def test_capacity_and_usage_limits_are_enforced() -> None:
    manager = ComputeNodeManager([
        node("full", current_jobs=1, max_concurrency=1, state=NodeState.ONLINE),
        node("quota", usage_limit_remaining=0, state=NodeState.ONLINE),
    ])
...
def test_private_node_can_be_used_without_storing_raw_secret() -> None:
    private = node("private-1", NodeKind.PRIVATE, credential_ref="secret-ref-123", state=NodeState.ONLINE)
...
def test_failover_excludes_failed_node() -> None:
    manager = ComputeNodeManager([
        node("a", state=NodeState.ONLINE),
        node("b", state=NodeState.ONLINE),
    ])
...
def test_release_cannot_make_job_count_negative() -> None:
    manager = ComputeNodeManager([node("a", state=NodeState.ONLINE)])
